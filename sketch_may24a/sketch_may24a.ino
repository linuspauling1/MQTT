...
char* topic_prag_inferior = "nodemcu/prag/inferior";//numele topicurilor pe care transmitem catre NodeMCU,
char* topic_prag_superior = "nodemcu/prag/superior";//temperaturile prag,
char* topic_diferenta = "nodemcu/diferenta";//diferenta maxima admisa intre valorile temperaturilor,
char* topic_perioada = "nodemcu/perioada";//perioada de achizitie a temperaturilor si
char* topic_1wire_rec = "nodemcu/1wire";//temperatura culeasa de senzorul 1-wire
char* topic_wifi_senzor1 = "temperaturi/wifi/senzor1";//numele topicurilor de pe care primim temperaturile
char* topic_wifi_senzor2 = "temperaturi/wifi/senzor2";
char* topic_1wire = "temperaturi/1wire";
...
int analogValue;//o folosim la achitizia de temperatura
...
void callback(const char topic[], byte* payload, unsigned int length) {
  ...
  } else if(!strcmp(topic,topic_1wire_rec)) {
    temperatura_1wire = atof(buffer);
  } else {
    Serial.println("Topic-ul este irelevant pentru cerinta.");
  }
  free(buffer);
}
...
void setup() {
  ...
  while(!client_mqtt.connect(clientID,NULL,NULL,"temperaturi/wifi/senzor1",2,true,"M-am deconectat...",false)) { //verificam conexiunea drept client MQTT
    Serial.print("Nu s-a conectat clientul MQTT...\n");
    mesaj_eroare();
    delay(100);//intarziere mai mica
  }
  while(!client_mqtt.subscribe("nodemcu/#",1)) { //ne abonam la canalele care ne ofera informatii
...
void actuatori() {
  ...
  float histerezis_cald_inferior = diferenta/2.f;
  float histerezis_cald_superior = diferenta/2.f;
  int dc;
  if(temperatura_1wire > prag_superior + histerezis_cald_superior)
    racitor = true;
  if((temperatura_1wire < prag_superior - histerezis_cald_inferior) && racitor)
    racitor = false;
  if(racitor){
...
void loop() {
  ...
  if(!flag_loop && (cur - prev_check >= 500)) {//ne reconectam din 500ms in 500ms
    ...
    if(!client_mqtt.subscribe("prag/#",1)) { //NU este necesara aceasta portiune de cod, ar fi fost doar in cazul in care 

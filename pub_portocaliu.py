...
while not my_sql_conexiune:
    try:
        ...
        time.sleep(1)
topic_perioada = 'nodemcu2/perioada'
topic_prag_inferior = 'nodemcu2/prag/inferior'
topic_prag_superior = 'nodemcu2/prag/superior'
topic_diferenta = 'nodemcu2/diferenta'
...
while not client.bad_connection_flag:
    if client.connected_flag:
        try:
            mydb.commit()
            my_cursor.execute('select perioada from camere where nume_camera = \'portocalie\'')#achizitie perioada
            try:
            ...
            my_cursor.execute('select prag_inferior from camere where nume_camera = \'portocalie\'')#achizitie prag inferior
            try:
            ...
            my_cursor.execute('select prag_superior from camere where nume_camera = \'portocalie\'')#achizitie prag superior
            try:
            ...
            my_cursor.execute('select diferenta from camere where nume_camera = \'portocalie\'')#achizitie diferenta
            try:

...
def esteNumar(numar):
    try:
        ...
        return False
q1 = Queue() #contine datele primite de la snezor1
q2 = Queue() #contine datele primite de la senzor2
q3 = Queue() #contine datele primite de la 1-wire
cliend_id = 'rpi_sub_portocaliu' #un client asemanator cu cel anterior
...
topic_senzori_senzor1 = 'temperaturi2/senzori/senzor1'
topic_senzori_senzor2 = 'temperaturi2/senzori/senzor2'
topic_senzori = 'temperaturi2/senzori/#'
#callbacks:
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        client.connected_flag = True
        print('M-am conectat.') #daca return code-ul este nul, atunci conexiunea este viabila
        try: #incercam sa ne abonam la canalele pentru senzorii care non 1-wire
...
def on_message(client, userdata, message):
    ...
    if message.topic == topic_senzori_senzor1:
        if esteNumar(payload):
            ...
            print('De la senzor1 avem: ',payload)
    elif message.topic == topic_senzori_senzor2:
        if esteNumar(payload):
            q2.put(payload)
        else:
            print('De la senzor2 avem: ',payload)
        ...
        while not q1.empty() and not q2.empty() and not q3.empty():
            ...
            try:
                ...
                try:
                    ...
                    values(\'portocalie\',%s, %s, %s)',(t1,t2,t3))
                except:
